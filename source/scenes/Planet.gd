...
func _process(delta):
	...
	light_direction = Vector3(marker.position.x, marker.position.y, 1.0).normalized()
	sprite.material.set_shader_parameter("light_direction", light_direction)
	if ring:
		ring.material.set_shader_parameter("light_direction", light_direction)
